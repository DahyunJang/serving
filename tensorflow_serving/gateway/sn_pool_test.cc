...
TEST(SnPool, SNDestruction) {
...

    {
        ...
        EXPECT_EQ(sn_pool.GetSN(ip_port1), sp_sn1);
        sn_pool.DestroySN(ip_port1);
    }
    EXPECT_NE(sp_sn1, nullptr);

    {
        SptrSN sp_sn2;
        sp_sn2 = sn_pool.CreateSN(ip_port2);
        EXPECT_NE(sn_pool.GetSN(ip_port2), sp_sn2);
        sp_sn2 = sn_pool.CreateSN(ip_port2);
        EXPECT_EQ(sp_sn2, nullptr);
    }
}
